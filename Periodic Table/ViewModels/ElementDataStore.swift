...
final class ElementDataStore: ObservableObject {
    ...
    func loadElements() async {
        ...
        do {
            // Prefer Bowserinator CSV (shells + bohr_model_3d URLs); fall back to bundled JSON
            if let csvElements = try? await PeriodicTableCSVLoader.loadElements(), !csvElements.isEmpty {
